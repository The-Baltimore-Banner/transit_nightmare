...
As part of our investigation 'TKTKTKT', we estimated each Baltimore City middle and high school student's trip to school on public transit.
...
# Broken tiger/lines version 
# make a join version with lat and lon for r5r
# bmore_tracts_join <-
#   bmore_tracts %>% 
#   clean_names() %>% 
#   dplyr::select(geoid20, name20, geometry) %>% 
#   rename(tract_name = name20) %>%
#   st_as_sf() %>% 
#   st_transform(4326) %>% 
#   st_difference(parks) %>% 
#   st_make_valid() %>%
...
# write_rds(travel_times_from_student_home_tracts, paste0("data/poa/", today, " copy/travel_times_df.rds"))
...
# latest_leaving_times_for_detailed_routing %>% 
#   write_rds("/Users/greg.morton/Documents/baltimoreBanner/Projects/2025/nicar_2025_transit/data/poa/2025-01-17/travel_times_western_01_17.rds")
```
...
#   #                             )) %>% 
#   all_trips %>% 
#   dplyr::select(census_tract, name, total_duration, everything()) 
...
unweighted_trip_summaries %>% 
  write_rds("data/output/unweighted_trip_summaries_01_17")
```
...
# longest_trip_every_school %>% 
#     write_csv("data/output/longest_trip_every_school.csv")
...
<!-- Do a thing for Ryan  -->
<!-- ```{r} -->
<!-- hs_parcels_buffer_crosswalk <-  -->
<!--   hs_parcels_buffer %>%  -->
<!--   dplyr::select(name, prg_num, address, geometry.x, geometry.y) %>%  -->
<!--   rename( -->
<!--     shape = geometry.x, -->
<!--     point = geometry.y -->
<!--   ) -->
...
<!--   enrollment_23 %>%  -->
<!--   dplyr::select(prog_no, program, enrollment) -->

<!-- thing_for_ryan <- detailed_trip_summaries_by_school %>%  -->
<!--   left_join(hs_parcels_buffer_crosswalk, by = c("prog_no" = "prg_num")) %>%  -->
<!--   left_join(enrollment_crosswalk, by = c("prog_no" = "prog_no")) %>%  -->
<!--   dplyr::select(name.x, name.y, program, everything()) %>%  -->
<!--   dplyr::select(-ends_with(".y")) %>%  -->
<!--   mutate(to_id = str_replace_all(stringr::str_to_lower(program), " ", "_")) %>%  -->
<!--   st_write("data/output/trip_summary_enrollment_and_shapes_for_ryan.geojson") -->

<!-- all_trips %>%  -->
<!--   summarise( -->
<!--     total = sum(students), -->
<!--     total_walking = sum(students[trip_legs == 1]) -->
<!--   ) -->
