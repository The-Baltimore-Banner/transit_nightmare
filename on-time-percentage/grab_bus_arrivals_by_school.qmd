...
dates <- seq(as.Date("2024-12-03"), as.Date("2025-01-14"), by = "day") %>% 
...
first_period_attendance_tabula <- 
  read_csv("/Users/greg.morton/Documents/baltimoreBanner/Projects/2024/high_school_transportation/data/raw/tabula-MPIA Response - Liz Bowie - Baltimore Banner - Attendance Data.csv", col_names = FALSE) %>% 
  rename(
    school = X1,
    first_period = X2,
...
school_buffers <- 
  ...
  # check if tract centroid is within the buffered zone
...
school_buffers[4,] %>% .$school_buffered %>% mapview::mapview() +
  school_buffers[4,] %>% .$tract_point %>% mapview::mapview() 
...
school_buffers %>% 
  ...
  # dplyr::select(name, enrollment, min_date:median_n_morning, within_pct) %>% 
  ...
  # write_csv(here::here("data", "output", "are_there_enough_buses_12_18.csv"))
  ...
  #   enrollment,
  #   median_n_morning,
  #   capacity_25_morning:capacity_45_morning,
  #   median_n_afternoon,
  #   capacity_25_afternoon:capacity_45_afternoon
  # ) %>% 
  # rename(
...
bus_arrivals_avg <- 
  ...
  summarise(
    ...
    # sd_morning = sd(avg_n_morning, na.rm = T),
    # variance_morning = var(avg_n_morning, na.rm = T),
    ...
    # sd_afternoon = sd(avg_n_afternoon, na.rm = T),
    # variance_afternoon = var(avg_n_afternoon, na.rm = T),
  ) 
...
are_there_enough_buses %>% 
  write_rds(here::here("data", "output", "are_there_enough_buses_within_30_02_14.rds"))
# 



  lm(capacity_25_morning ~ first_period, data = are_there_enough_buses) %>% summary()
