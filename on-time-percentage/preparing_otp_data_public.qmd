---
title: "Preparing OTP Data"
editor: visual
---

## Get the remaining files we need from S3

This script is going to check our bus locations folder in the desktop and ensure that each day folder has hours between 4 and 23 and each hour folder inside each day folder has between 710 and 725 json files.

I suspect we have almost everything, but let's see which ones we still need to go and grab by hand.
...
```{r}
# Helper function to create date-based directories
...
# Function to determine the date from a file path
extract_date <- function(file_path) {
  # Assuming the date is in YYYY-MM-DD format somewhere in the path
  date_pattern <- "\\d{4}-\\d{2}-\\d{2}"
  date_match <- regexpr(date_pattern, file_path)
  if (date_match != -1) {
    return(substr(file_path, date_match, date_match + 9))
  }
  return(NULL)
}
...
flatten_json_df <- function(data) {
  if (!is.data.frame(data)) {
    data <- as.data.frame(data)
  }

  # Identify list columns that need flattening
  list_cols <- sapply(data, is.list)

  # Flatten each list column
  for (col in names(data)[list_cols]) {
    # Convert lists to character if they can't be easily flattened
    if (all(sapply(data[[col]], length) == 1)) {
...
dates_for_processing <- 
  seq.Date(from = as.Date("2024-12-03"), to = as.Date("2025-01-14"), by = "day") %>%
  .[! . %in% to_rm] %>% 
  .[! . %in% weekend] %>% 
  as.character()
...
# would not recommend you storing bus location data in the same repo as this script
...
read_and_standardize <- function(file, all_columns) {
  ...
  if(!dir.exists(paste0("/Users/your.name/Desktop/bus-positions/school-transportation/processed_bus_data/", today, "/", file_date))) {
    
    # replace path with your own
    dir.create(paste0("/Users/your.name/Desktop/bus-positions/school-transportation/processed_bus_data/",today, "/", file_date), recursive = TRUE)
  ...
  write_rds(data, paste0("/Users/your.name/Desktop/bus-positions/school-transportation/processed_bus_data/", today, "/", file_date, "/", file_date, "-", file_time, ".rds"))
...
process_bus_data_by_day <- 
  ...
  files <- list.files(paste0("/Users/your.name/Desktop/bus-positions/school-transportation/processed_bus_data/", today, "/", date), full.names = TRUE, pattern = "\\.rds$", recursive = TRUE) 
